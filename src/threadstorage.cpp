...
QMap<QString, ConversationThread> ThreadStorage::loadAllThreads(const QString &projectId)
{
    ...
    // Determine which threads to load based on projectId parameter
    QStringList fullThreadIds;
    QString prefix;
    
    if (projectId.isEmpty()) {
        // Load all threads (no project filter)
        fullThreadIds = m_storage->listThreads();
        QString currentProjectId = ThreadJsonStorage::getCurrentProjectId();
        prefix = ThreadJsonStorage::getProjectPrefix(currentProjectId);
    } else {
        // Load threads for specific project
        QString effectiveProjectId = projectId;
        prefix = ThreadJsonStorage::getProjectPrefix(effectiveProjectId);
        fullThreadIds = m_storage->listThreadsForProject(effectiveProjectId);
    }
